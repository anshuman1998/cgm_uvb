...
def make_many_plots(den, met, outpath, ion_num = [8], uvb_true ='KS18', q_true = 18):
    ...
    x = [-1000, -1000]
    ax1.scatter(x, x, alpha=0.5, s=18, label='Using {} ions'.format(num), color = 'b')
    ...
    ax1.set_xlim(np.log10(den)-add_num0, np.log10(den)+add_num)
    #print()
    ax1.set_ylim(met -add_num0 , met+add_num)
    ax2.set_xlim(np.log10(den)-add_num0,  np.log10(den)+add_num)
    ax3.set_xlim(met -add_num_met0, met+add_num_met)
    ax2.set_ylim(0, 174)
    ax3.set_ylim(0, 174)
