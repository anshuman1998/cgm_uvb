...
def make_many_plots(den, met, outpath, ion_num = [3, 5, 8]):
    # setting the figure
    ...
    out_fig_name = outpath + '/fig_photo_scatter__logZ{}_logN{}.jpg'.format(met, np.log10(den))
    figure_size = [14, 4]
    ...
    for num, col in zip(ion_num, color_list):
        ...
        # sort_d = sort_d[sort_d['z_dmax']>1.5]

        # print(sort_d)

        n_max_array = sort_d['n_dmax']
        z_max_array = sort_d['z_dmax']

        ax1.scatter(n_max_array, z_max_array, alpha=0.5, label='{} ions'.format(num), s=13)

        binwidth = 0.02

        ax2.hist(n_max_array, bins=np.arange(min(n_max_array), max(n_max_array) + binwidth, binwidth), alpha=0.75,
                 histtype='bar', edgecolor='k', linewidth=1.5, density=1)

        ax3.hist(z_max_array, bins=np.arange(min(z_max_array), max(z_max_array) + binwidth, binwidth), alpha=0.75,
                 histtype='bar', edgecolor='k', linewidth=1.5, density=1)

        n_med = np.median(n_max_array)
        z_med = np.median(z_max_array)

        label_name = r'$\Delta_{\rm max}$ (log n$_{\rm H}$) =' + '{:.2f} ->{}ions'.format(n_med, num)
        ax2.axvline(n_med, linestyle='--', label=label_name)
        label_name = r'$\Delta_{\rm max}$ (log Z) =' + '{:.2f} ->{}ions'.format(z_med, num)
        ax3.axvline(z_med, linestyle='--', label=label_name)
        ...
        print(np.median(n_max_array), np.median(z_max_array), 'ions', num)
    ...
    ax1.annotate(r'True (log Z, log n$_{\rm H}$) = ' + '({:.0f}, {:.0f})'.format(met, np.log10(den)),
                 xy=(0.06, 0.77), xycoords='axes fraction', fontsize=12)
