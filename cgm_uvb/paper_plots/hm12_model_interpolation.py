...
z  =  np.genfromtxt("UVB_hm12.out", skip_header= 20, max_rows=1)
data  =  np.loadtxt("UVB_hm12.out", skiprows = 21)
wave = data[:, 0]
data_new =  data[:, 1:]
...
for z_val in my_z:
    redshift = z_val
    new_file_name = folder_to_store + '/HM12_EBL_z_{:.2f}.fits'.format(redshift)
