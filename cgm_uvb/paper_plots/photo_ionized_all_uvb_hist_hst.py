...
def make_many_plots(den, met, outpath, ion_num = [5], uvb_true ='KS18', q_true = 18):
    ...
    for the_uvb, q_val in zip(uvb_models, the_Q_values):
        ...
        n_max_array = np.random.choice(n_max_array, 56, replace = False)
        z_max_array = np.random.choice(z_max_array, 56, replace = False)
        ...
        if the_uvb != 'P19' and the_uvb !='FG20':
            txt = 'Q{}'.format(q_val)
            if q_val <=15:
                ax1.annotate(txt, (n_med- 0.03, z_med + 0.085), fontsize=8, color = new_colr)
            else:
