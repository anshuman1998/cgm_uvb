...
out_fig_name = 'scatter_diff_ion_hybrid_NH4_bimodal_both.pdf'
figure_size = [14, 4]
...
for num, colr in zip(ion_num, color_list):
    ...
    ax1.scatter(n2_all_new_high, z2_all_new_high, alpha=0.5, label='', s=13, color = colr)

    ax2.hist(n2_all_new_high, bins=np.arange(min(n2_all_new_high), max(n2_all_new_high) + binwidth, binwidth), alpha = 0.75,
             histtype = 'bar', edgecolor= 'k', linewidth = 1.5, density = 1, label = '', color = colr )
    ax3.hist(z2_all_new_high, bins=np.arange(min(z2_all_new_high), max(z2_all_new_high) + binwidth, binwidth), alpha = 0.8,
             histtype = 'bar',  edgecolor= 'k', linewidth = 1.5, density = 1, label = '', color  = colr)
