...
def combine_hybrid_output(logT, path = '/home/vikram/cloudy_run/diff_op/hybrid_NH15' ):

    # standard uvb ran for this project
    ...
    # to get structure of first four 'common' columns
    q = 18
    uvb = 'KS18'
    ...
    final_filename = path + '/all_combined_logT{:.0f}.fits'.format(logT * 100)
    new_tab.write(final_filename, overwrite=True)

print('---------running now for T = 10^5.75 K')
combine_hybrid_output(logT=5.75)
print('---------running now for T = 10^6 K')
combine_hybrid_output(logT=6.0)