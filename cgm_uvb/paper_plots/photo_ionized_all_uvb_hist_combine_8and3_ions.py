...
def make_many_plots(den, met, outpath, path = '/home/vikram/cloudy_run/diff_op/photo_NH19', ion_num = [3, 8], uvb_true ='KS18', q_true = 18):
    ...
    num=ion_num[0]
    dnum0 = d[d['n_ions'] == num]
    sort_d0 = dnum0[dnum0['true_nH'] == den]
    sort_d0 = sort_d0[sort_d0['true_logZ'] == met]
    ...
    for the_uvb, q_val in zip(uvb_models, the_Q_values):
        ...
        n_max_array0 = sort_d0[col_name_nH]
        z_max_array0 = sort_d0[col_name_Z]
        ...
        ax1.scatter(n_max_array, z_max_array, alpha=0.3, facecolor = 'None', s=13, edgecolor  = new_colr_alp, linewidth = 0.6)


        #ax1.scatter(n_max_array, z_max_array, color = 'k', alpha = 0.8, s = 13, facecolor ='None')
        #ax1.scatter(n_max_array0, z_max_array0, marker ='x', s=10, color = new_colr)
        ax1.scatter(n_max_array0, z_max_array0, marker ='x', s=7, color  = new_colr, linewidth = 0.1)
        ...
        ax2.hist(n_max_array0, bins=np.arange(min(n_max_array0)-0.5*binwidth, max(n_max_array0) + 1.5*binwidth, binwidth),
                 histtype='step', linewidth=1.2, edgecolor = new_colr)

        ax3.hist(z_max_array0, bins=np.arange(min(z_max_array0)-0.5*binwidth, max(z_max_array0) + 1.5*binwidth, binwidth),
                 histtype='step', linewidth=1.2, edgecolor = new_colr)
        ...
        txt = '{:.2f}'.format(n_med)
        if the_uvb != 'P19':
            ax2.annotate(txt, (n_med+0.008, 150), fontsize=8, color = new_colr)
        else:
            ax2.annotate(txt, (n_med+0.008, 135), fontsize=8, color = new_colr)
    ...
    x = [-1000, -1000]
    ax1.scatter(x, x, alpha=0.5, s=18, label='Using 8 ions', color = 'b')
    ax1.scatter(x, x, alpha=0.5, s=15, label='Using 3 ions', color = 'b', marker  = 'x')
