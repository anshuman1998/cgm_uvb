...
out_fig_name = 'scatter_den_photoionized_all.pdf'
figure_size = [14, 4]
...
for den, col in zip(n_H_array, color_list):
    ...
    ax1.plot ([np.median(n2_all_new), np.median(n2_all_new), 0], [0, np.median(z2_all_new),  np.median(z2_all_new)],
...
for ax in(ax1, ax2, ax3):
    ...
    if ax ==ax1:
        ax.annotate('Photoionized\n'+ 'absorber \n' + '(using 5 ions)', xy=(0.05, 0.80), xycoords='axes fraction', fontsize=12)
