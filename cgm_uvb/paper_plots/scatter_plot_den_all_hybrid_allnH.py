...
out_fig_name = 'scatter_den_hybrid_all_nions.pdf'
figure_size = [14, 4]
...
for den, col in zip(n_H_array, color_list):
        ...
        for m, n, Z, in zip(d['Q'][d['true_nH'] == den], d['Max_diff_nH'][d['true_nH'] == den],
            ...
            for uvb in all_uvb:
                if uvb == m:
                    ...
                    n2_all.append(n)
                    z2_all.append(Z)
        print(len(n2_all_new), ': total points')

    label ='true (log Z, log' + r' n$_{\rm H}$' + ') = (-1, {:.1f})'.format(np.log10(den))
    ax1.scatter(n2_all_new, z2_all_new, alpha = 0.4, label =label, s = 16, facecolor = col, edgecolor = col)
    #ax1.scatter(n2_all_new, z2_all_new, alpha = 0.4, label ='', s = 16, facecolor = 'none', edgecolor = col)
    #ax1.scatter(n2_all_new, z2_all_new, alpha = 0.45, label =label, s = 16, facecolor = col, edgecolor = col)
    #ax1.scatter(n2_all_new, z2_all_new, alpha = 0.9, label =label, s = 16, facecolor = 'none', edgecolor = col)

    ax1.scatter( np.median(n2_all_new), np.median(z2_all_new), marker = '*', s = 300, color = 'red', edgecolor = 'k', zorder =10)
    ax1.plot ([np.median(n2_all_new), np.median(n2_all_new), 0], [-0.05, np.median(z2_all_new),  np.median(z2_all_new)],
    ...
    ax2.axvline(np.median(n2_all_new), linestyle='--', color=col, linewidth=2, label=median_lab, zorder =10)
    median_lab = r'$\Delta_{\rm max}$ (log Z)' + '\n = {:.2f}'.format(np.median(z2_all_new))
    ax3.axvline(np.median(z2_all_new), linestyle='--', color=col, linewidth=2, label=median_lab, zorder =10)
...
for ax in(ax1, ax2, ax3):
    ...
    if ax ==ax1:
        ax.annotate('Hybrid\n'+ 'absorber' +r' (10$^{5.5}$ K)', xy=(0.05, 0.87),
