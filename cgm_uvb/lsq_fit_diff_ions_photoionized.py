...
#----model interpolation
def get_interp_func_for_all_photo(model_path, ions_to_use, Q_uvb = 18, uvb = 'KS18'):
...
#def get_intrp_function_2D(model_path, set_of_all_useful_ions, model_uvb, Q_uvb = 18):
    # get interpolation functions for all ions

#    interp_logf = get_interp_func_for_all_photo(model_path=model_path, ions_to_use=set_of_all_useful_ions, Q_uvb=Q_uvb, uvb=model_uvb)
...
def inference_for_photoionized_cloud(model_path, true_nH_array= [1e-4], true_logZ_array = [-1], number_of_ions_array = [4],
    ...
    full_interp_function_list = get_interp_func_for_all_photo(model_path=model_path, ions_to_use=qualified_ion_list,
                                                              Q_uvb=model_Q, uvb= model_uvb)
    ...
    filename_string = 'photo_model_{}_Q{}_lsf_inference'.format(model_uvb, model_Q)
