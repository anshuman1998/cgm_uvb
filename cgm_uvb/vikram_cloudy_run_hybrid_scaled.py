...
def run_parallel(uvb_Q, uvb, scale_value, logT = 5.5):
    # for vikram
    cloudy_path = '/home/vikram/c17.02'
    fname = logT*10
    input_File = '/home/vikram/cloudy_run/rescaled_hybrid_NH15/try_{}_Q{}_logT{:.0f}.in'.format(uvb, uvb_Q, fname)
    print(uvb, 'Q=', uvb_Q, 'T=', logT)

    # write input file and run cloudy
    ions, params = cloudy_params_defaults(uvb = uvb, uvb_Q=uvb_Q, log_hden=[-6, -2, 0.02], stop_NHI = 15,
                                          T = 10**logT, uvb_scale = scale_value, sequential = True)
