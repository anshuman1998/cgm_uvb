...
def run_parallel(logZ, uvb_Q, uvb):
    ...
    fname = (logZ+4)*100
    input_File = '/scratch/vikram/cloudy_run/metal_NH16/try_{}_Q{}_Z{:.0f}.in'.format(uvb, uvb_Q, fname)
    print(uvb, 'Q=', uvb_Q, 'Z=', logZ)

    # write input file and run cloudy
    ions, params = cloudy_params_defaults(uvb = uvb, uvb_Q=uvb_Q, log_hden=[-6, -2, 0.02], stop_NHI = 16, T = None, metal = logZ,
                                          sequential = True)
