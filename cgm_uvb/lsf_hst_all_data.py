...
def inference_for_photoionized_cloud():
    ...
    # eight selected ions
    qualified_ion_list = ['C+', 'C+3', 'N+', 'N+4', 'O+5', 'Si+', 'Si+2', 'Si+3']
    data_col_all_obs_val = [13.85, 13.23, 13.63, 13.07, 13.88, 13.21, 12.93, 12.68]

    #qualified_ion_list = ['C+', 'C+3', 'N+', 'N+4',  'Si+', 'Si+2', 'Si+3']
    #data_col_all_obs_val = [13.85, 13.23, 13.63, 13.07,  13.21, 12.93, 12.68]
    ...
    data_col_all = tab.Table(np.array([data_col_all_obs_val]), names = np.array(qualified_ion_list))

    print(qualified_ion_list)


    # interpolating grid
    lognH_array = np.arange(-5, -1.999, 0.01) # grid in log nH
    logZ_array = np.arange(-3, 1.0001, 0.01)
    number_nH = len(lognH_array)
    number_Z = len(logZ_array)
    ...
    for model_Q in mod_Q:
        ...
        model_path = '/home/vikram/Work/my_papers/proposals_for_grants_and_time/hst31/PG1116_Model_fits/Q{}_try.fits'.format(model_Q)
        ...
        filename_string = 'pg116_Q{}.fits'.format(model_Q)
        ...
        for ions_to_use in ion_set:
            ...
            final_n.append(inferred_nH)
            final_z.append(inferred_Z)
            ...
            #make_nH_true_array.append(true_nH)
            #make_Z_true_array.append(true_logZ)
            #print(':->', model_Q, 'nH:', inferred_nH, 'Z:', inferred_Z, 'num_ions:', len(ions_to_use))


        res = tab.Table([final_n, final_z], names = ('nH', 'Z'))

        path  = '/home/vikram/cgm_uvb/hst/'
        res.write(path + filename_string, overwrite  = True)
        print(res)
