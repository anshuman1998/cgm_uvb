...
#----for mcmc
def log_likelihood(theta, interp_logf, obs_ion_col, col_err, model_Q, reference_log_metal = -1.0):
    """
...
def log_posterior(theta, interp_func, data_col, sigma_col, model_Q):
    log_p = log_prior(theta) + \
            log_likelihood(theta, interp_logf = interp_func, obs_ion_col = data_col, col_err = sigma_col, model_Q = model_Q)
...
def run_mcmc(model_Q, ions_to_use, true_Q =18 ):
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior, args=(interp_logf, data_col, sigma_col, model_Q))
